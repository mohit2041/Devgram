...
router.get('/:id', auth, async (req, res) => {
    try {
        ...
        if(err.kind == 'ObjectId'){
            return res.status(400).json({ msg: 'Post not found , check postID is correct?' });
        }
        console.error(err.message);
...
router.delete('/:id',auth, async (req, res) => {
    try {
        ...
        if(err.kind == 'ObjectId'){
            return res.status(400).json({ msg: 'Post not found , check params-ID is correct?' });
        }
        console.error(err.message);
...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            // if post is not available with the above ID
            if (!post) {
                return res.status(404).json({ msg: 'Post not found' });
            }
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
    try {
        ...
        // check user of comment
        if(comment.user.toString() !== req.user.id){
            return res.status(401).json({ msg: 'User not authorised' });
        }
        

        // finding remove index and deleting the comment
        ...
        // if id is not the type of ObjectId
        if(err.kind == 'ObjectId'){
            return res.status(400).json({ msg: 'Post/comment not found , check params-ID is correct?' });
        }
